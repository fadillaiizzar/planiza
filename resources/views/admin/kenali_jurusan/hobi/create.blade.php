<div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-none">
    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-white">Tambah Data Hobi</h2>
            </div>
        </div>
    </div>
    ...
    <div class="px-6 pt-2 pb-6">
        <form action="{{ route('admin.kenali-jurusan.hobi.store') }}" method="POST" class="space-y-4">
            @csrf

            <!-- Nama Hobi -->
            <div class="space-y-1">
                <label for="nama_hobi" class="block text-sm font-semibold text-slate-700">Nama Hobi</label>
                <input type="text" name="nama_hobi" id="nama_hobi"
                    value="{{ old('nama_hobi') }}" placeholder="Masukkan nama hobi"
