    ...
    <section class="bg-off-white rounded-2xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <x-admin.statistic-card
                icon="fas fa-book"
                :label="$labelJurusan"
                :items="$hobiPerJurusan"
            />

            <x-admin.statistic-card
                icon="fas fa-heart"
                :label="$labelHobi"
                :items="$jurusanPerHobi"
            />
        </div>
    </section>
            ...
            <table class="w-full text-left text-sm table-auto">
                ...
                <tbody>
                    @forelse ($items as $relasi)
                        <tr class="border-b border-border-gray hover:bg-off-white/50 transition-colors hobi-jurusan-row"
                            data-hobi="{{ strtolower($relasi->hobi->nama_hobi) }}"
                            data-jurusan="{{ strtolower($relasi->jurusanKuliah->nama_jurusan_kuliah) }}">
                            <td class="p-4 w-[5%]">{{ $relasi->id }}</td>
                            <td class="p-4 w-[30%]">{{ Str::limit($relasi->hobi->nama_hobi, 50) }}</td>
                            <td class="p-4 w-[30%]">{{ Str::limit($relasi->jurusanKuliah->nama_jurusan_kuliah, 50) }}</td>
                            <td class="p-4 w-[20%]">{{ $relasi->poin }}</td>
                            <td class="p-4 relative overflow-visible w-[10%]">
                                ...
                                <div id="dropdown-{{ $relasi->id }}" class="hidden absolute right-14 mt-2 bg-white border border-border-gray rounded-lg shadow-xl z-20 min-w-[180px] overflow-visible">
                                    <a href="{{ route('admin.kenali-jurusan.hobi-jurusan.show', $relasi->id) }}"
                                    ...
                                    <form action="{{ route('admin.kenali-jurusan.hobi-jurusan.destroy', $relasi->id) }}" method="POST" class="w-full">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            onclick="showDeleteRelasiModal(
                                                {{ $relasi->id }},
                                                '{{ addslashes($relasi->hobi->nama_hobi) }}',
                                                '{{ addslashes($relasi->jurusanKuliah->nama_jurusan_kuliah) }}',
                                                '{{ route('admin.kenali-jurusan.hobi-jurusan.destroy', $relasi->id) }}'
                                            )"
                    ...
                    @empty
                        <x-empty-state
                            ...
                            message="Belum ada relasi hobi ↔ jurusan. Tambahkan data baru"
                            button="true"
...
@include('admin.kenali_jurusan.hobi_jurusan.delete')

<script>
    function showEdit(id) {
        fetch(`/admin/kenali-jurusan/hobi-jurusan/${id}/edit`)
            .then(res => res.text())
            .then(html => {
