<div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-none">
    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-4">
        <div class="flex items-center space-x-3">
            ...
            <h2 class="text-lg font-semibold text-white">Tambah Kategori SDGs</h2>
        </div>
    </div>
    ...
    <div class="px-6 pt-2 pb-6">
        <form action="{{ route('admin.sdgs.kategori-sdgs.store') }}" method="POST" class="space-y-4">
            @csrf

            <!-- Nomor Kategori -->
            <div class="space-y-1">
                <label for="nomor_kategori" class="block text-sm font-semibold text-slate-700">Nomor Kategori (1–17)</label>
                <input type="number" name="nomor_kategori" id="nomor_kategori" min="1" max="17"
                    value="{{ old('nomor_kategori') }}" placeholder="Masukkan nomor kategori SDGs"
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="nama_kategori" id="nama_kategori"
                    value="{{ old('nama_kategori') }}" placeholder="Masukkan nama kategori SDGs"
                    class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white" required>
            </div>
