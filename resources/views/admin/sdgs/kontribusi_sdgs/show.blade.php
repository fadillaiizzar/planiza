    ...
    <main class="flex-1 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6 md:p-8">
        ...
        <div class="mb-8 max-w-5xl mx-auto">
            <nav class="flex items-center space-x-2 text-sm">
                ...
                class="group flex items-center px-4 py-2 rounded-full text-slate-400 hover:text-slate-600 hover:bg-white/70 transition-all">
                    <i class="fas fa-hands-helping w-4 h-4 mr-2 text-slate-300 group-hover:text-slate-500"></i>
        ...
        <div class="w-full max-w-5xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
            ...
            <div class="px-8 pt-8 pb-4 space-y-6">
                ...
                <div>
                    <label class="font-semibold text-sm text-slate-700 mb-2 flex items-center">
                        <i class="fas fa-paperclip mr-2 text-teal-500"></i> Bukti Upload
                    </label>
                    ...
                    <div class="p-4 bg-white border rounded-2xl shadow-sm">
                        ...
                        @if ($item->bukti_upload)
                            ...
                            @if (in_array($ext, ['jpg','jpeg','png','gif','webp']))
                                <img src="{{ Storage::url($path) }}" class="rounded-xl border max-h-72 object-contain">

                                <a href="{{ Storage::url($path) }}"
                                    class="px-4 py-2 mt-3 bg-slate-navy text-white rounded-xl shadow font-medium inline-block"
                                    download>
                                    <i class="fas fa-download mr-1"></i> Download Gambar
                                </a>
                            @else
                                <a href="{{ Storage::url($path) }}"
                                class="px-4 py-2 bg-slate-navy text-white rounded-xl shadow font-medium inline-block"
                                download>
                        ...
                        @else
                            <span class="text-slate-500 italic">Tidak ada bukti upload</span>
                        @endif
                    </div>
                </div>
