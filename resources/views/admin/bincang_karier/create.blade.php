<div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-none">
    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-white">Tambah Pertanyaan Bincang Karier</h2>
            </div>
        </div>
    </div>
    ...
    <div class="px-6 pt-2 pb-6">
        <form action="{{ route('admin.bincang-karier.store') }}" method="POST" class="space-y-4">
            @csrf

            <!-- Isi Pertanyaan -->
            <div class="space-y-1">
                <label for="isi_pertanyaan" class="block text-sm font-semibold text-slate-700">Pertanyaan</label>
                <textarea name="isi_pertanyaan" id="isi_pertanyaan" rows="4"
                    ...
                    placeholder="Tulis pertanyaan..." required>{{ old('isi_pertanyaan') }}</textarea>
            </div>
