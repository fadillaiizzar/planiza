@if ($item->user_id === Auth::id())
    <div id="editBincangModal-{{ $item->id }}"
        ...
        <div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200
                    ...
                    <div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M11 4h10M3 4h4m14 0l-4 14H7L3 4m8 0v14" />
                        </svg>
                    </div>
            ...
            <div class="px-6 pt-4 pb-6">
                ...
                <form method="POST"
                    action="{{ route('admin.bincang-karier.update', $item->id) }}"
                    class="space-y-4">
                    @csrf
                    @method('PUT')

                    <!-- Textarea (isi awal dipertahankan) -->
                    <div class="space-y-1">
                        <label class="block text-sm font-semibold text-slate-700">Pertanyaan</label>

                        <textarea name="isi_pertanyaan" rows="5" required
                                class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white focus:ring-2 focus:ring-slate-300 focus:outline-none">{{ old('isi_pertanyaan', $item->isi_pertanyaan) }}</textarea>
                    ...
                    <div class="flex items-center justify-center pt-4 gap-5">
                        <button type="button"
                                onclick="closeActionModal('editBincangModal-{{ $item->id }}')"
