<div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-none">
    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-white">Tambah Jurusan Kuliah</h2>
            </div>
        </div>
    </div>
    ...
    <div class="px-6 pt-2 pb-6">
        <form action="{{ route('admin.eksplorasi-jurusan.jurusan-kuliah.store') }}" method="POST" class="space-y-4" enctype="multipart/form-data">
            @csrf

            <!-- Nama Jurusan -->
            <div class="space-y-1">
                <label for="nama_jurusan_kuliah" class="block text-sm font-semibold text-slate-700">Nama Jurusan</label>
                <input type="text" name="nama_jurusan_kuliah" id="nama_jurusan_kuliah"
                    value="{{ old('nama_jurusan_kuliah') }}" placeholder="Masukkan Nama Jurusan"
            ...
            <div class="space-y-1">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                    class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white"
                    placeholder="Masukkan deskripsi jurusan..." required>{{ old('deskripsi') }}</textarea>
            </div>

            <!-- Mata Kuliah -->
            <div class="space-y-1">
                <label for="info_matkul" class="block text-sm font-semibold text-slate-700">Mata Kuliah</label>
                <textarea name="info_matkul" id="info_matkul" rows="2"
                    class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white"
                    placeholder="Mata kuliah yang dipelajari..." required>{{ old('info_matkul') }}</textarea>
            </div>

            <!-- Prospek -->
            <div class="space-y-1">
                <label for="info_prospek" class="block text-sm font-semibold text-slate-700">Prospek</label>
                <textarea name="info_prospek" id="info_prospek" rows="2"
                    class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white"
                    placeholder="Prospek karir atau pekerjaan..." required>{{ old('info_prospek') }}</textarea>
            </div>
