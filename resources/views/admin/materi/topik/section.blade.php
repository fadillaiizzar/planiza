                        ...
                        >
                            ...
                            <td class="p-4 relative overflow-visible">
                                ...
                                <div id="dropdown-{{ $item->id }}" class="hidden absolute right-20 mt-2 bg-white border border-border-gray rounded-lg shadow-xl z-20 min-w-[180px] overflow-visible">
                                    <a href="{{ route('admin.pembelajaran.topik-materi.show', $item->id) }}"
                                    ...
                                    <div class="border-t border-border-gray"></div>
                                    <button type="button" onclick="showDeleteModal({{ $item->id }}, '{{ addslashes($item->judul_topik) }}', '{{ route('admin.pembelajaran.topik-materi.destroy', $item->id) }}')"
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl p-8 w-full max-w-lg shadow-2xl border border-[#CBD5E1] relative overflow-hidden">
        ...
        <div class="text-center">
            ...
            <h3 class="text-xl font-bold text-[#1E293B] mb-2">Hapus Topik Materi</h3>
            <p class="text-[#64748B] mb-6 leading-relaxed text-sm sm:text-base">
                Apakah Anda yakin ingin menghapus topik materi berjudul
...
<script>
    ...
    function showEdit(id) {
        fetch(`/admin/pembelajaran/topik-materi/${id}/edit`)
            .then(res => res.text())
            .then(html => {
