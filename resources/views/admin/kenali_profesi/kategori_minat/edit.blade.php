<div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-none">
    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-white">Edit Kategori Minat</h2>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.kenali-profesi.kategori-minat.update', $kategoriMinat->id) }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="nama_kategori" id="nama_kategori"
                       value="{{ old('nama_kategori', $kategoriMinat->nama_kategori) }}"
            ...
            <div class="space-y-1">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Masukkan deskripsi kategori"
                          class="w-full px-3 py-2 border border-slate-300 rounded-xl text-sm text-slate-900 bg-white" required>{{ old('deskripsi', $kategoriMinat->deskripsi) }}</textarea>
            </div>
