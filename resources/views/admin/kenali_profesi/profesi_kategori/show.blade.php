...
@section('title', 'Detail Relasi Profesi - Kategori Minat - Planiza')
...
<main class="flex-1 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6 md:p-8">
    <div class="mb-8 max-w-4xl mx-auto">
        ...
        <nav class="flex items-center space-x-2 text-sm">
            <a href="{{ route('admin.kenali-profesi.profesi-kategori.index') }}"
               class="group flex items-center px-4 py-2 rounded-full text-slate-400 hover:text-slate-600 hover:bg-white/70 transition-all duration-200">
                ...
                <span class="font-medium">Relasi Profesi - Kategori Minat</span>
            </a>
    ...
    <div class="w-full max-w-4xl mx-auto text-left bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-slate-600 to-slate-700 px-8 py-6">
            <div class="flex items-center space-x-3 text-left justify-start">
                ...
                <div>
                    <h2 class="text-xl font-bold text-white">Detail Relasi Profesi - Kategori Minat</h2>
                    <p class="text-slate-200 text-sm">Informasi lengkap hubungan profesi dengan kategori minat</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-8 pt-8 pb-0 space-y-5">
            ...
            <div class="group">
                <label class="block text-sm font-semibold text-slate-700 mb-3 flex items-center">
                    <i class="fas fa-user-tie mr-2 text-green-500"></i> Profesi
                </label>
                <div class="w-full px-5 py-4 border border-slate-200 rounded-2xl text-sm text-slate-900 bg-white shadow-sm">
                    {{ $profesiKategori->profesiKerja->nama_profesi_kerja ?? '-' }}
                </div>
            </div>
            ...
            <div class="group">
                <label class="block text-sm font-semibold text-slate-700 mb-3 flex items-center">
                    <i class="fas fa-th-large mr-2 text-purple-500"></i> Kategori Minat
                </label>
                <div class="w-full px-5 py-4 border border-slate-200 rounded-2xl text-sm text-slate-900 bg-white shadow-sm">
                    {{ $profesiKategori->kategoriMinat->nama_kategori ?? '-' }}
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between text-left">
                <div class="text-xs text-slate-500">
                    <span class="flex items-center">
                        ...
                        Terakhir diperbarui: {{ $profesiKategori->updated_at ? $profesiKategori->updated_at->format('d M Y, H:i') : '-' }}
                    </span>
                </div>
            </div>
        </div>
